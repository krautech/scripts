...
${NC}"
printf "${RED}Firmware Updater Script ${NC} v0.1.1\n"
...
probe_dfu(){
	...
	wget https://apdm.tech/Full_Cartographer_CAN_1M_4_0_1.bin
	options=("Yes Continue" "No")
	echo "Firmware File: Full_Cartographer_CAN_1M_4_0_1.bin"
	echo "Do you wish to proceed with flashing your cartographer probe using DFU Mode?"
	select opt in "${options[@]}"; do
		case $REPLY in
			1) sudo dfu-util -R -a 0 -s 0x08000000:leave -D Full_Cartographer_CAN_1M_4_0_1.bin; break ;;
			2) break 2 ;;
			*) echo "What's that?" >&2
		esac
	done
...
while true; do
	...
	if [[ $saved_uuid == "" ]] && [[ $deviceid == "" ]] && [[ $usbid == "" ]]; then
		options=("Check DFU Mode" "Quit")
		select opt in "${options[@]}"; do
			case $REPLY in
				1) check_dfu; break ;;
				2) sudo service klipper start; exit; break ;;
				*) echo "What's that?" >&2
			esac
		done
	elif [[ $deviceid != "" ]]; then
		options=("Check DFU Mode" "Flash Cartographer CANBUS ONLY - via DFU" "Quit")
		select opt in "${options[@]}"; do
			case $REPLY in
				1) check_dfu; break ;;
				2) probe_dfu; break ;;
				3) sudo service klipper start; exit; break ;;
				*) echo "What's that?" >&2
			esac
		done
